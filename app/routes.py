...
def order_detail(order_num):
    ...
    order_to_update = Order.query.filter_by(id=order_num).first()
    order_to_update.order_total = total
    db.session.commit()
    return render_template('your_order_details.html', data=order_placed)


@app.route('/order_cancel/<int:order_num>', methods=['GET', 'DELETE'])
def delete_order(order_num):
    order_to_delete = Order.query.filter_by(id=order_num).first()
